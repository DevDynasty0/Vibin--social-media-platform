  ...
  const handlePostSubmit = async (e) => {
    ...
    try {
      const res = await axios.post(
        "https://vibin-social-media-platform-backend.onrender.com/api/v1/posts/post",
        // { newPost, formData },
        newPost,
        {
          headers: {
...
<div className="">
    <div className="text-center mb-6"><h1 className="text-color-one font-bold text-2xl font-sans "> Vibin <span className="bg-black text-white h-4 w-4 p-1 rounded">Ai</span> </h1>
       ...
       {buttonSpinner ? (
    <div className=" pt-[10%] items-center xl:max-w-[55%] lg:[70%] w-[90%] min-h-[35vh]  h-full relative bg-[#DEBBDF]  mx-auto flex flex-col justify-between p-4    rounded-t-lg shadow"> <Spinner color="blue.500"  /></div>
) : (
       ...
       <div className="p-5">
        ...
        <div className={`${generatedCaption.length > 0 ? "block" : "hidden"}`}>
           ...
           <div className={`${generatedImage.length > 0 ? "block" : "hidden"}`}>
               ...
               <img className="mt-1 h-[30%] w-full mx-auto" src={generatedImage} onLoad={(e) => setImageSource(e.target.value)} alt="" />
     </div>
    
            {generatedCaption.length==0 && generatedImage.length == 0 && (
        <div className="text-center  h-[35vh] mt-[4%] text-color-one text-md lg:text-xl">Hi {user.fullName}! Confused how to express your vibe?<br />Let me help you</div>
      )}
...
<div className="flex gap-5 mt-2 items-center">
        ...
        <button className={`btn btn-sm bg-gray-300 text-gray-400 py-1 px-2 text-xs md:text-sm rounded-md ${activeGenerator === "caption" ? "bg-green-500 text-white" : ""}`} onClick={() => setActiveGenerator("caption")}>Caption Generator</button>
                    ...
                    <button className={`btn btn-sm bg-gray-300 text-gray-400 py-1 px-2 text-xs md:text-sm rounded-md ${activeGenerator === "image" ? "bg-green-500 text-white" : ""}`} onClick={() => setActiveGenerator("image")}>Image Generator</button>
                  
                <div onClick={handleRefresh } className="flex items-center gap-2 text-white hover:bg-blue-200 hover:text-black md:bg-red-500  py-1 px-2 rounded-md "> <button className="hidden md:block md:text-sm" >Refresh</button> < LuRefreshCcw className="text-lg md:text-white text-black"></LuRefreshCcw></div>
                </div>
       ...
       <div className="flex flex-row items-center">
         <div className="flex flex-row items-center w-full border rounded-3xl border-gray-500  h-12 px-2">
                   ...
                   {activeGenerator === "caption" ? (
                     <textarea name="userPrompt" className="border border-transparent  bg-transparent  w-full focus:outline-none text-sm h-10 flex items-center" type="text" placeholder="Give your prompt for caption" />
                    ) : (   
                      <textarea name="userPrompt" className="border border-transparent bg-transparent w-full focus:outline-none text-sm h-10 flex items-center" type="text" placeholder="Give your prompt for image" />
                      )}     
          </div>
        ...
        <div className="xl:max-w-[55%] lg:[70%] w-[90%] mt-[1%] mx-auto">
                        ...
                        <div onClick={handlePostSubmit}  className=" cursor-pointer  px-3 py-2 text-sm font-medium text-center text-white bg-color-one rounded-lg hover:bg-violet-500 ">
                            Drop Vibe
                        </div>
