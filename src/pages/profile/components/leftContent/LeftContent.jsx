...
export default function LeftContent({userProfile}) {
  ...
  return (
    <div className="relative p-4 text-center ">
      <div className="relative">
        <img className='object-cover w-44  h-44 block mx-auto md:-mt-[50%] -mt-[30%] rounded-full' src={userProfile?.avatar || 'https://res.cloudinary.com/dsfyrjd8b/image/upload/v1707303580/by2cegfudppucnxbwvun.png'} alt="Profile" />
        ...
        <div className="mb-5 mt-10">
          <h2 className="font-bold md:text-2xl text-[10px]">{userProfile?.fullName}</h2>
          <span className="text-[14px] font-bold">@{userProfile?.userName}</span>
        </div>
        ...
        <div className='flex items-center gap-4 justify-center bg-white rounded-md px-2 py-6'>
          ...
          <div className=" items-center">
            ...
            <h3 className="font-bold text-[10px] md:text-sm">{userProfile?.followersCount
}</h3>
          </div>
        </div>
       ...
       <FaUniversity className='text-2xl bg-slate-400 rounded-full w-7 p-1 h-7 '></FaUniversity>
          <p> <span className='font-bold ml-1'></span> {userProfile?.university}</p>
       </div>
       <div className='flex items-center my-2 flex-col '>
       <FaHome className='text-2xl bg-slate-400 rounded-full w-7 p-1 h-7 '></FaHome>
          <p> <span className='font-bold ml-1 mr-3 '></span> {userProfile?.address}</p>
       </div>
        </div>
      </div>
    </div>
