...
export default function LeftContent({user,profileRefetch }) {
  const [profileImage, setProfileImage] = useState('');
  // const [selectedImage, setSelectedImage] = useState('');
  console.log('lefftttttttt',user);
  const handleImageChange = async (avatar) => {
    try {
      ...
      console.log('leftcontent',response);
      ...
      <div className="relative">
        ...
        <div className='flex items-center gap-4 justify-center bg-white rounded-md px-2 py-6'>
          ...
          <div className=" items-center">
            ...
            <h3 className="font-bold text-[10px] md:text-sm">{user?.data?.followersCount
}</h3>
          </div>
        </div>
       ...
       <FaUniversity className='text-2xl bg-slate-400 rounded-full w-7 p-1 h-7 '></FaUniversity>
          <p> <span className='font-bold ml-1'></span> {user?.data?.university}</p>
       </div>
       <div className='flex items-center my-2 flex-col '>
       <FaHome className='text-2xl bg-slate-400 rounded-full w-7 p-1 h-7 '></FaHome>
          <p> <span className='font-bold ml-1 mr-3 '></span> {user?.data?.address}</p>
       </div>
        </div>
      </div>
    </div>
